...
public class lugares_especificos extends Activity{
	...
	static String url_ruta_corta_2 = url+"WebService/destino.php";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int bandera_out_in = bundle.getInt("bandera");
		...
		if(bandera_out_in == 1){
			...
			new load_lugares_especificos_2().execute(descripcion, bssid_menor);
		}
	}
	...
	class load_lugares_especificos_2 extends AsyncTask<String, String, String>{
		@Override
		protected void onPreExecute(){
    		super.onPreExecute();
    		pDialog = new ProgressDialog(lugares_especificos.this);
		...
		protected String doInBackground(String... params) {
			...
			parametros.add(new BasicNameValuePair(TAG_VALUE1, params[0]));
			parametros.add(new BasicNameValuePair(TAG_VALUE2, params[1]));
			JSONObject json = JParser.makeHttpRequest(url_ruta_corta_2, "POST", parametros);
			Log.v("Destino: ", json.toString());
			try{
				int success = json.getInt(TAG_SUCCESS);
				if(success == 1){
					bandera = 1;
